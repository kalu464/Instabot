import asyncio, json, os, random, time, io, tempfile
...
# ---------------------------
# Helper: write bytes to temp file (not strictly needed but kept for compatibility)
# ---------------------------
...
async def _pfp_worker(bot_instance, chat_id, img_bytes):
    ...
    try:
        while True:
            try:
                # Use an in-memory bytes buffer; name attribute helps PTB detect filename sometimes
                bio = io.BytesIO(img_bytes)
                bio.name = "pfp.jpg"
                bio.seek(0)
                # set_chat_photo is an async call
                await bot_instance.set_chat_photo(chat_id, bio)
            except asyncio.CancelledError:
                # allow graceful cancel
                break
            except Exception as e:
                # catch other errors and retry after short sleep
        ...
        # cleanup: remove task entry if exists
