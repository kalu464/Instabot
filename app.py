...
import platform # Added for system specs
import ast # Added for parsing imports
...
# --- Configuration ---
TOKEN = '8233030913:AAEwx_BYpyzcfB7qDAcEEq6EDxAJY_yL12U' # Replace with your actual token
OWNER_ID = 5915051224 # Replace with your Owner ID
ADMIN_ID = 5915051224 # Replace with your Admin ID (can be same as Owner)
YOUR_USERNAME = '@pvtt_prix' # Replace with your Telegram username (without the @)
UPDATE_CHANNEL = '@prix_era' # Replace with your update channel link

# Folder setup - using absolute paths
BASE_DIR = os.path.abspath(os.path.dirname(__file__)) # Get script's directory
UPLOAD_BOTS_DIR = os.path.join(BASE_DIR, 'upload_bots')
PENDING_DIR = os.path.join(BASE_DIR, 'pending_uploads') # Directory for pending files
...
os.makedirs(PENDING_DIR, exist_ok=True) # Create pending directory
    ...
    try:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS pending_files
                     (id INTEGER PRIMARY KEY AUTOINCREMENT,
                      ...
                      file_path TEXT,
                      file_type TEXT,
                      message_id INTEGER)''') # Added pending_files table
...
def get_system_specs():
    try:
        ...
        return (f"💻 **System Specifications**\n\n"
                f"🖥️ **OS**: {os_info}\n"
                f"🧠 **CPU**: {cores_physical} Cores ({cores_logical} Threads) | Usage: {cpu_usage}%\n"
                f"💾 **RAM**: {ram_used_gb}/{ram_total_gb} GB ({ram_percent}%)\n"
                f"💽 **Storage**: {disk_used_gb}/{disk_total_gb} GB ({disk_percent}%)")
    except Exception as e:
        logger.error(f"Error getting system specs: {e}")
        return "❌ Error retrieving system specifications."

def install_requirements_from_file(file_path, message_obj_for_reply):
    try:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
        
        # Use AST to find imports for better accuracy
        imports = set()
        try:
            ...
            for node in ast.walk(tree):
             ...
             logger.warning(f"SyntaxError parsing {file_path} for imports. Falling back to regex.")
             # Fallback to Regex if AST fails (e.g. invalid syntax)
        ...
        # Filter standard library modules roughly (not exhaustive, but helpful)
        sys_modules = set(sys.builtin_module_names)
        
        installed_count = 0
        if imports:
            bot.reply_to(message_obj_for_reply, f"🔍 Analyzing imports: found {len(imports)} potential modules...", parse_mode='Markdown')
            for module in imports:
                if module in sys.modules or module in sys_modules: continue 
                # Check if installed via pip freeze (expensive but accurate) or just try install
                # For speed, we'll use attempt_install_pip which installs if missing
                # But attempt_install_pip currently doesn't check if installed, it just runs pip install
                # Pip install will skip if already satisfied, so it's safe.
                if attempt_install_pip(module, message_obj_for_reply):
                    installed_count += 1
            
            if installed_count > 0:
                bot.reply_to(message_obj_for_reply, f"✅ Auto-installed {installed_count} packages required by the script.")
        return True
    except Exception as e:
        logger.error(f"Error in auto-install requirements: {e}")
        return False
...
def save_pending_file_db(user_id, file_name, file_path, file_type, message_id):
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
            c.execute('INSERT INTO pending_files (user_id, file_name, file_path, file_type, message_id) VALUES (?, ?, ?, ?, ?)',
                      (user_id, file_name, file_path, file_type, message_id))
            conn.commit()
            return c.lastrowid
        except Exception as e:
            logger.error(f"❌ Error saving pending file: {e}", exc_info=True)
            return None
        finally:
            conn.close()

def get_pending_file_db(pending_id):
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
            c.execute('SELECT user_id, file_name, file_path, file_type, message_id FROM pending_files WHERE id = ?', (pending_id,))
            return c.fetchone()
        except Exception as e:
            logger.error(f"❌ Error getting pending file: {e}", exc_info=True)
            return None
        finally:
            conn.close()

def delete_pending_file_db(pending_id):
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
            c.execute('DELETE FROM pending_files WHERE id = ?', (pending_id,))
            conn.commit()
        except Exception as e:
            logger.error(f"❌ Error deleting pending file: {e}", exc_info=True)
        finally:
            conn.close()
...
def create_main_menu_inline(user_id):
    ...
    buttons = [
        ...
        types.InlineKeyboardButton('📦 Install Package', callback_data='install_package'),
    ...
    if user_id in admin_ids:
        ...
        markup.add(buttons[3], buttons[4]) # Speed, Install Package
        markup.add(admin_buttons[0], admin_buttons[1]) # Subscriptions, Stats
        markup.add(admin_buttons[3], admin_buttons[2]) # Broadcast, Lock Bot
        markup.add(admin_buttons[5], admin_buttons[4]) # Run All Scripts, Admin Panel
    ...
    try:
        ...
        if req_file:
            ...
            try:
             ...
             logger.info("No requirements.txt found, scanning .py files for imports...")
             for py_file in py_files:
                 if py_file == '__init__.py': continue
                 full_py_path = os.path.join(temp_dir, py_file)
                 install_requirements_from_file(full_py_path, message)
...
def handle_py_file(file_path, script_owner_id, user_folder, file_name, message):
    try:
        ...
        install_requirements_from_file(file_path, message)
...
def _logic_statistics(message):
    ...
    # Re-calculating running bots
    for script_key_iter, script_info_iter in list(bot_scripts.items()):
        if script_info_iter.get('process') and script_info_iter['process'].is_running():
             running_bots_count += 1
    
    # System Specs
    specs = get_system_specs()

    stats_msg_base = (f"{specs}\n\n"
                      f"📊 **Bot Statistics**:\n"
...
def handle_file_upload_doc(message): # Renamed
    ...
    try:
        ...
        pending_filename = f"{user_id}_{timestamp}_{file_name}"
        pending_path = os.path.join(PENDING_DIR, pending_filename)
        
        with open(pending_path, 'wb') as f:
            f.write(downloaded_file_content)
        
        logger.info(f"Saved pending file to {pending_path}")
        
        # Save to DB first to get ID
        pending_id = save_pending_file_db(user_id, file_name, pending_path, file_ext, 0)
        ...
        if pending_id:
            ...
            try:
                # Send the file to the owner for inspection
                # We send the file from disk to ensure filename is correct and content is fresh
                try:
                    with open(pending_path, 'rb') as f_doc:
                        bot.send_document(OWNER_ID, f_doc, visible_file_name=file_name, caption=f"📄 **File Content**\n📂 `{file_name}`\n👤 User: `{user_id}`", parse_mode='Markdown')
                except Exception as e:
                    logger.error(f"Failed to send document: {e}")
                    # Fallback to sending by file_id if local file fails (unlikely)
                    bot.send_document(OWNER_ID, doc.file_id, caption=f"📄 **File Content (via ID)**\n📂 `{file_name}`\n👤 User: `{user_id}`", parse_mode='Markdown')
                ...
                bot.send_message(
                    ...
                    f"👆 The file has been forwarded above for your review.",
                    parse_mode='Markdown',
                    reply_markup=markup
                )
                bot.reply_to(message, f"✅ File `{file_name}` uploaded successfully!\n\n⚠️ **Pending Approval**: This file has been sent to the Owner for security verification. It will be hosted automatically once approved.")
            except Exception as e:
                logger.error(f"Failed to send approval request to owner: {e}")
                bot.reply_to(message, "❌ Error sending file for approval. Please contact owner.")
        else:
            bot.reply_to(message, "❌ Database error saving pending file.")
...
class DummyMessage:
    def __init__(self, user_id):
        ...
        self.message_id = None # Reply will become a normal message

def approve_callback(call):
    try:
        pending_id_str = call.data.split('_')[1]
        pending_id = int(pending_id_str)
        pending_file = get_pending_file_db(pending_id)
        
        if not pending_file:
            bot.answer_callback_query(call.id, "⚠️ Pending file not found.", show_alert=True)
            try: bot.delete_message(call.message.chat.id, call.message.message_id)
            except: pass
            return

        user_id, file_name, pending_path, file_ext, _ = pending_file
        
        if not os.path.exists(pending_path):
             bot.answer_callback_query(call.id, "❌ Pending file missing from disk.", show_alert=True)
             delete_pending_file_db(pending_id)
             return
        ...
        try:
            if file_ext == '.zip':
                handle_zip_file(file_content, file_name, dummy_message)
            else:
                ...
                if file_ext == '.js':
                    handle_js_file(dest_path, user_id, user_folder, file_name, dummy_message)
                elif file_ext == '.py':
                    handle_py_file(dest_path, user_id, user_folder, file_name, dummy_message)
            
            bot.send_message(user_id, f"✅ Your file `{file_name}` has been **APPROVED** by the Owner!", parse_mode='Markdown')
            ...
            logger.error(f"Error processing approved file: {e}", exc_info=True)
            bot.send_message(OWNER_ID, f"❌ Error processing approved file `{file_name}`: {e}")

    except Exception as e:
        logger.error(f"Error in approve_callback: {e}", exc_info=True)
        bot.answer_callback_query(call.id, "Error approving file.")
...
def reject_callback(call):
    try:
        ...
        user_id, file_name, pending_path, _, _ = pending_file
        
        bot.answer_callback_query(call.id, f"❌ Rejecting {file_name}...")
        
        bot.send_message(user_id, f"❌ Your file `{file_name}` has been **REJECTED** by the Owner.", parse_mode='Markdown')
        ...
        logger.error(f"Error in reject_callback: {e}", exc_info=True)
        bot.answer_callback_query(call.id, "Error rejecting file.")

def install_package_callback(call):
    bot.answer_callback_query(call.id)
    msg = bot.send_message(call.message.chat.id, "📦 Enter the package name to install (e.g., `requests`, `numpy`).\n/cancel to abort.")
    bot.register_next_step_handler(msg, process_package_name)
...
def process_package_name(message):
    ...
    if not re.match(r'^[a-zA-Z0-9_\-]+$', package_name):
         bot.reply_to(message, "⚠️ Invalid characters in package name. Use only letters, numbers, underscores, and dashes.")
         return

    attempt_install_pip(package_name, message)
...
def handle_callbacks(call):
    ...
    try:
        ...
        elif data.startswith('approve_'): owner_required_callback(call, approve_callback) # Added
        elif data.startswith('reject_'): owner_required_callback(call, reject_callback)   # Added
        ...
        elif data == 'install_package': install_package_callback(call)
